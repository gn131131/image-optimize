            ...
            <div style={{ maxWidth: 960, margin: "0 auto", padding: 24 }}>
                    ...
                    <div style={{ marginTop: 24 }}>
                        ...
                        <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
                            ...
                            <label>
                                <input
                                    type="checkbox"
                                    checked={clientResize}
                                    onChange={(e) => setClientResize(e.target.checked)}
                                />{" "}
                                客户端预处理
                            </label>
                            {clientResize && (
                                <span style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                    ...
                                    <span>
                                        ...
                                        <input
                                            ...
                                            value={preQuality}
                                            style={{ width: 70 }}
                                            onChange={(e) => setPreQuality(Number(e.target.value))}
                                        />
                                    </span>
                                    <button
                                        type="button"
                                        ...
                                        onClick={async () => {
                                            ...
                                            const result = await preprocessImages(
                                                files,
                                                { maxWidth, maxHeight, quality: preQuality, format: "auto" },
                                                (d, t) => setPreProgress({ done: d, total: t })
                                            );
                                            const reduced = result.reduce(
                                                (s, r) => s + (r.originalSize - r.processedSize),
                                                0
                                            );
                            ...
                            <button
                                disabled={!images.length && !files.length}
                                onClick={downloadZip}
                            ...
                            {preFiles && (
                                <span style={{ fontSize: 12, color: "#58a6ff" }}>
                                    已预处理节省 {formatSize(preReducedBytes)}
                                </span>
                            )}
                ...
                {summary && (
                    <div style={{ marginTop: 24 }}>
                        <h2>汇总</h2>
                        <p>
                            原始总大小: {formatSize(summary.totalOriginal)} → 优化后:{" "}
                            {formatSize(summary.totalOptimized)}
                        </p>
                        <p>
                ...
                {images.length > 0 && (
                    <div style={{ marginTop: 32 }}>
                        ...
                        >
                            {images.map((img, idx) => (
                                <div
                                    key={img.downloadName}
                                    style={{ border: "1px solid #30363d", padding: 12, borderRadius: 8 }}
                                >
                                    ...
                                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                                        {img.variants?.map((v) => (
                                            <div key={v.format} style={{ textAlign: "center" }}>
                                                <img
                                                    src={v.base64}
                                                    style={{ maxWidth: 120, cursor: "zoom-in" }}
